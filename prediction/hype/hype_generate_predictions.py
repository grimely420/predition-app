#!/usr/bin/env python3
"""
HYPE prediction generator - calls API every 5 minutes.
"""
...
from hype.config import API_HOST, API_PORT, PREDICTION_HORIZON, SYMBOL

# Setup logging
logger = setup_logging("HYPE-Generator")
...
def make_prediction() -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                logger.info(
                    f"Prediction: ${data.get('predicted_price_5min', data.get('predicted_price', 0)):,.4f} "
                    f"(Current: ${data['current_price']:,.4f}, "
